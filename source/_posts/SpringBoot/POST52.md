---
title: Spring Boot微服框架学习(一)
tags: SpringBoot
category: SpringBoot
abbrlink: 48216
date: 2017-12-20 01:34:36
---
![image](http://ovi3ob9p4.bkt.clouddn.com/TIETU/CT0025.jpg)
...
![image](http://ovi3ob9p4.bkt.clouddn.com/springboot/springboot003.png)
...
并不是每个人都喜欢继承自spring-boot-starter-parentPOM。您可能有您需要使用的自己的公司标准parent，或者您可能更喜欢显式声明所有的Maven配置。如果你不想使用spring-boot-starter-parent，您仍然可以通过使用scope = import依赖关系来保持依赖关系管理：
...
```
# application.properties配置和使用

> 摘要: Spring Boot使用“约定优于配置”（项目中存在大量配置，此外还内置了一个约定的配置，让你无需手动进行配置）的理念让你的项目快速运行起来。所以，我们想要熟练使用Spring Boot，就要懂得如何开启各个功能模块的默认配置，这就需要了解Spring Boot的配置文件application.properties。
...
![image](http://ovi3ob9p4.bkt.clouddn.com/springboot/springboot004.jpg)
...
## 实现方式
使用@ControllerAdvice或@RestControllerAdvice注解
使用@ExceptionHandler注解

通过使用@ControllerAdvice或@RestControllerAdvice定义统一的异常处理类。@ExceptionHandler用来定义函数针对的异常类型， 自定义错误处理器，可自己组装json字符串，并返回 。
...
import com.nxtcec.ordering.mlsoa.exception.SoaException;
import com.nxtcec.ordering.mlsoa.model.BodyStatus;
import com.nxtcec.ordering.mlsoa.model.BodyValidStatus;
import com.nxtcec.ordering.mlsoa.util.Constants;
import com.nxtcec.ordering.mlsoa.util.DataUtil;
...
public class GlobalExceptionHandler {
	...
	public ResponseEntity handle(Exception e) {
		...
		if (e instanceof SoaException) {
			bodyStatus = ((SoaException) e).getBodyStatus();
		} else if (e instanceof MissingServletRequestParameterException) {
			bodyStatus = DataUtil.bodyStatus("0002");
		} else if (e instanceof HttpRequestMethodNotSupportedException) {
			bodyStatus = DataUtil.bodyStatus("0003");
		} else if (e instanceof HttpMediaTypeNotSupportedException) {
			bodyStatus = DataUtil.bodyStatus("0004");
		} else if (e instanceof HttpMessageNotReadableException) {
			bodyStatus = DataUtil.bodyStatus("0005");
		} else if (e instanceof BindException) {
			bodyStatus = DataUtil.bodyStatus("0006");
		} else if (e instanceof NumberFormatException) {
			bodyStatus = DataUtil.bodyStatus("0007");
		} else if (e instanceof DataIntegrityViolationException) {
			bodyStatus = DataUtil.bodyStatus("0008");
		} else {
...
public class SoaException extends RuntimeException {
	...
	private static final long serialVersionUID = -2945796190275982393L;
	...
	public SoaException(String code) {
		this.bodyStatus = DataUtil.bodyStatus(code);
	}

	public SoaException(String code, String message) {
		this.bodyStatus = DataUtil.bodyStatus(code, message);
	}
...
![image](http://ovi3ob9p4.bkt.clouddn.com/springboot/springboot005.jpg)
